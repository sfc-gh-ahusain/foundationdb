...
public:
	...
	AsyncTrigger waitForRecruitment;
	AsyncTrigger waitForRemoteRecruitment;
	RecruitFromConfigurationReply recruitmentReply;
	RecruitRemoteFromConfigurationReply remoteRecruitmentReply;
	...
	ClusterControllerData(ClusterControllerFullInterface const& ccInterface,
	  ...
	  : gotProcessClasses(false), gotFullyRecoveredConfig(false), shouldCommitSuicide(false),
	    ...
	    versionDifferenceUpdated(false), recruitDistributor(false), recruitRatekeeper(false),
...
struct ClusterRecoveryData : NonCopyable, ReferenceCounted<ClusterRecoveryData> {
	...
	ClusterRecoveryData(ClusterControllerData* controllerData,
	  ...
	  : controllerData(controllerData), dbgid(masterInterface.id()), lastEpochEnd(invalidVersion),
	    ...
	    primaryLocality(tagLocalityInvalid), cc("ClusterController", dbgid.toString()),
...
ACTOR Future<Void> newTLogServers(Reference<ClusterRecoveryData> self,
	...
	if (self->configuration.usableRegions > 1) {
		...
		state Reference<RecruitRemoteWorkersInfo> recruitWorkersInfo = makeReference<RecruitRemoteWorkersInfo>(remoteRecruitReq);
		...
		/*
		Future<RecruitRemoteFromConfigurationReply> fRemoteWorkers =
		    self->controllerData->findRemoteWorkersForConfiguration(RecruitRemoteFromConfigurationRequest(remoteRecruitReq));
		*/
	...
	TraceEvent("NewTLogServers end", self->dbgid).log();
	return Void();
}
			...
			try {
				...
				if (conf != self->configuration) {
					if (self->recoveryState != RecoveryState::ALL_LOGS_RECRUITED &&
					    self->recoveryState != RecoveryState::FULLY_RECOVERED) {
						TraceEvent("ConfigurationMonitor kill-self", self->dbgid).log();
						self->controllerData->shouldCommitSuicide = true;
...
ACTOR Future<Void> handleLeaderReplacement(Reference<ClusterControllerRecovery::ClusterRecoveryData> self,
	...
	loop choose {
		when(wait(leaderFail)) {
			TraceEvent("Leader Replaced", self->controllerData->id).log();
...
ACTOR Future<Void> clusterWatchDatabase(ClusterControllerData* cluster,
	...
	loop {
		...
		try {
			...
			recoveryData =
			    makeReference<ClusterControllerRecovery::ClusterRecoveryData>(cluster,
			                                                                  db->serverInfo,
	...
	for (int i = 0; i < self->outstandingRecruitmentRequests.size(); i++) {
		...
		try {
			...
			if (info->dbgId.present()) {
				TraceEvent("CheckOutstandingRecruitmentReq", info->dbgId.get())
...
void checkOutstandingRemoteRecruitmentRequests(ClusterControllerData* self) {
	for (int i = 0; i < self->outstandingRemoteRecruitmentRequests.size(); i++) {
		...
		try {
			...
			if (info->dbgId.present()) {
				TraceEvent("CheckOutstandingRemoteRecruitmentReq", info->dbgId.get())
				    .detail("Request", info->req.configuration.toString());
			}
			info->waitForCompletion.trigger();
