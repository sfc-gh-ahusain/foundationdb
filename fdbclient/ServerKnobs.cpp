...
void ServerKnobs::initialize(Randomize randomize, ClientKnobs* clientKnobs, IsSimulated isSimulated) {
	...
	*/
	...
	// encrypt key proxy
	init( ENABLE_ENCRYPTION,                                   true ); //if ( randomize && BUGGIFY ) { ENABLE_ENCRYPTION = deterministicRandom()->coinflip(); }
	...
	init( ENABLE_BLOB_FILE_ENCRYPTION,             ENABLE_ENCRYPTION ); 

	// Compression
	init( ENABLE_BLOB_FILE_COMPRESSION,                        false ); 
